...
| REQ-TRX-LIVE-04 | バックグラウンドスレッドが 1 秒周期でポーリングし、各ソースにつき 20 秒分（16kHz 換算）バッファが溜まったらまとめて Whisper に投入する。1 回のポーリングで、取り出せるチャンクをすべて処理する | `TranscriptionService.TranscriptionLoop`, `TakeNextChunk` |
