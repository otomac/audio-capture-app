...
| ID | 要件 | 実装箇所 |
|---|---|---|
| REQ-DEV-01 | 起動時および「更新」操作時に、有効な録音（Capture）デバイスと再生（Render）デバイスの一覧を取得できる。**「更新」の導線はメインウィンドウの入力カードに置き、マイク／スピーカーの選択の 2 行の直下・区切り線の上に右寄せで並べる**（REQ-SETWIN-01）。デバイスを差し替えたときにだけ使う操作ではあるが、**取り直した一覧から選び直すのはメインウィンドウのコンボボックスであり、「取り直す」と「選ぶ」が別ウィンドウに分かれると導線が切れる**ためである。実行できる条件は REQ-REC-09 と同じ（録音中・停止処理中・ファイル文字起こし中は無効） | `AudioCaptureService.RefreshDevices`, `EnumerateDevices`, `MainWindow.xaml`, `MainViewModel.RefreshDevicesCommand` |
...
| REQ-TRX-FILE-07 | 処理中に「中止」操作でキャンセルできる。キャンセル時は生成中の出力ファイルを削除し、部分結果を残さない。**削除してよいのはその実行が作りかけたファイルだけである。**話者ダイアライゼーション有効時はマージが終わるまで出力ファイルを開かないため、中止時点では未作成であり削除しない（無条件に削除すると前回成功したときの `.transcript.txt` を巻き添えにする）。また有効時の書き出しは推論がすべて終わった後の確定処理なので途中でキャンセルせず、全行書くか一行も書かないかのどちらかにする。**「中止」はオプション指定ダイアログにのみ置く。** メインウィンドウには置かない — ダイアログはモーダルで、処理中にメインウィンドウを操作できる場面が無いためである（ダイアログを閉じた場合は REQ-TRX-FILE-13 によりその時点で中止される）。**「中止」を押したことは、その場で見て分かるようにする** — 押した直後にボタンを無効化し、進捗表示の下に「中止を要求しました」の 1 行を出す。中止は推論の境界でしか評価されない（REQ-TRX-DIA-12）ため、**話者ダイアライゼーションが有効なら押してから実際に止まるまで数十秒〜数分かかり、その間も進捗は進み続ける**。この 1 行が無いと、押せたのか効いていないのかが分からない。**文言は「押した時点でどのフェーズにいるか」で変える**（REQ-TRX-FILE-06 のフェーズ名を見る） — 「話者識別中」なら「中止を要求しました（話者識別が終わるまでお待ちください）」、それ以外（Whisper の「処理中」・最初の進捗が届く前の準備中）なら「中止を要求しました（処理の切れ目までお待ちください）」。**話者ダイアライゼーションが有効かどうかで決めてはならない** — 有効時は「話者識別中」→「処理中」の 2 フェーズを通るため、Whisper 側で押したのに「話者識別が終わるまで」と出ることになり事実に反する（T160）。待たされる理由も長さもフェーズで違う: 話者識別は推論の境界でしか止まらず数十秒〜数分かかるのに対し、Whisper はチャンクと有声区間の境目ごとにキャンセルを見るので数秒で止まる。**文言は押した時点で確定させ、以後書き換えない** — 押した後にフェーズが進むことは無い（話者識別中に押せば、その完了直後の境界で中止が効いて Whisper には入らない）。**この 1 行は進捗のフェーズ表示（REQ-TRX-FILE-06）とは別の行に置く** — 同じ行に書くと次の進捗通知で上書きされるためである。**中止がいつ効くかという判定そのものは変えない**（REQ-TRX-DIA-12 を維持） | `MainViewModel.CancelFileTranscription`, `IsFileTranscriptionCancelRequested`, `FileTranscriptionCancelNotice`, `IsDiarizationPhase`, `TranscriptionService.TranscribeFileAsync` (catch `OperationCanceledException`), `FileTranscriptionOptionsWindow.xaml` |
...
| REQ-TRX-FILE-13 | 処理中にダイアログを閉じようとした場合、**閉じる前に「中止して閉じるか」を確認する。**はいならその時点で中止し（REQ-TRX-FILE-07 と同じ中止処理）、ダイアログを閉じる。いいえなら閉じない。確認を挟むのは、「キャンセル」ボタンが `IsCancel` であるため **Esc でも閉じうる**からで、確認が無いと長い文字起こしが誤操作で黙って捨てられる。**中止の完了は待たずに閉じる** — 中止はsherpa-onnx の推論境界でしか効かない（REQ-TRX-DIA-12）ため、待つとダイアログが固まる。閉じたあと中止が完了するまでの経過はステータスバーの 1 行で分かる | `FileTranscriptionOptionsWindow.xaml.cs` (`Window_Closing`), `MainViewModel.FileTranscriptionCloseConfirmation` / `CancelFileTranscriptionCommand` |
| REQ-TRX-FILE-14 | ファイル文字起こし中にメインウィンドウを閉じようとした場合、閉じる前に「中止して終了するか」を確認する。**いいえならウィンドウを閉じない**。はいの場合は中止処理（REQ-TRX-FILE-07 = 生成中の出力ファイルの削除）を**完了させてから**終了する。中止は sherpa-onnx の推論境界でしか効かないため（REQ-TRX-DIA-12）、話者ダイアライゼーション有効時は実行中の推論が終わるまで待つ。**この経路に入るのは、オプション指定ダイアログを閉じた後、中止が完了しきる前の間だけである** — ダイアログはモーダルなので開いている間はメインウィンドウを操作できず、閉じた時点で中止が始まる（REQ-TRX-FILE-13）。稀ではあるが、長い音声の推論境界を待っている間は現実に起こりうる | `MainWindow.MainWindow_Closing`, `MainViewModel.CloseConfirmationMessage` / `ShutdownAsync` |
...
| ID | 要件 | 実装箇所 |
|---|---|---|
| REQ-SETWIN-01 | メインウィンドウは**録音するための操作面**に絞り、常時表示しなくてよい設定は設定ウィンドウへ置く。メインウィンドウに残すのは、録音のたびに触るもの — **入力デバイスの選択（REQ-DEV-03 / 06）とその一覧の「更新」（REQ-DEV-01）・ミュート・レベルメーター・録音の開始／停止・経過時間・ライブ文字起こしの ON/OFF・「表示」（REQ-LIVEVIEW-01）・「音声ファイルから文字起こし」（REQ-TRX-FILE-01）・ステータスバー** に限る。**「更新」をここに含めるのは、押した結果を受け取るコンボボックスが同じカードの中にあるからである**（REQ-DEV-01） | `MainWindow.xaml`, `SettingsWindow.xaml` |
...
| REQ-SETWIN-03 | 設定ウィンドウに置くのは **①保存先フォルダ（表示と選択。REQ-CFG-03）／②Whisper モデルのパスと選択（REQ-TRX-01）／③ライブ文字起こしの言語（REQ-TRX-LIVE-14）／④文字起こしに GPU を使用する（REQ-GPU-02）** の 4 つと、**⑤話者ダイアライゼーションの状態**（REQ-TRX-DIA-15 の詳細。ステータスバーの短い表示を補う読み取り専用の説明であり、切り替えはできない）。**デバイス一覧の「更新」はここに置かない** — 選び直す先のコンボボックスから離れるためで、置き場はメインウィンドウである（REQ-DEV-01） | `SettingsWindow.xaml` |
