...
- [ ] **T141** 1 つの Whisper セグメントの中で話者が切り替わる場合に、テキストを分割できない（例: 「あマジで」＋別話者の発話が 1 行に同居）。分割を見送ったのは、Whisper の BPE トークンがマルチバイト文字の途中で切れると `WhisperToken.Text` の時点で U+FFFD に置換され、**元のバイトが復元できない**ため（実測: 18 セグメント中 9 件で発生）。素朴に連結すると半数が文字化けする。実装するなら `segment.Text` にカーソルを進め、壊れていないトークンの位置でのみ分割を許し、壊れたトークンの連なりは分割不可の塊として次の健全なトークンのテキスト検索で再同期する必要がある。**なお T137 の実測で、実機で観測された混在 3 件には本件は効かないと判明した** — 切り分ける先の話者境界がそもそも無い（検出漏れ）か、重なっている（同時発話）ためである（→ T142）。本件が効くのは「1 セグメント内に実際の話者境界がある」場合に限られ、その頻度は未測定。**まず頻度を測ってから、[低] の事象に見合うか判断する。** T137 の調査で発見
- [ ] **T142** 話者の割り当てを誤る箇所の真因が Diarization 側にあると判明した（T137 の実測）。2 つある。**①検出漏れ** — 短い割り込み（`#8` 15:08:16「えー、どれまでですか?」）に対応する話者区間が 1 つも生成されず、`15:08:15.03-15:08:28.97 話者1` の一本に飲まれている。`MinDurationOn` を 0.3→0.2 にしても全 58 行のラベルは変わらなかった（T115 のレビューで実測済み）。**②同時発話** — 話者区間どうしが時間的に重なる場合（`#13` は話者1 13.9 秒に話者4 0.32 秒が重なる、`#50` は話者30 と話者8 が重なる）、「最大重複の話者を選ぶ」規則では長い方が必ず勝ち、短い割り込み側は原理的に選ばれない。REQ-TRX-DIA-05 の規則そのものが同時発話を想定していない。**まず実機で再現させ、①②のどちらがどれだけ効いているかを切り分けてから対策を選ぶ**（segmentation モデルの差し替え／重なり時の規則の見直し／重なり区間を話者不明にする、等が候補）。T137 の実測で発見
