...
## 未着手
- [ ] **T125** 短い物音が結合幅（既定 2.0 秒）内に複数あると 1 区間にまとまり、大半が無音のまま Whisper に渡る（例: 0.1 秒のクリック音 2 つが 1.5 秒の区間になる）。有声「密度」による足切りを検討する。T112 のレビューで発見。※ 単純に「span ではなく有声合計で判定」に変えても、窓量子化により結合区間は必ず 2 窓 = 0.2 秒以上になるため挙動は変わらない
- [ ] **T126** `ProcessChunk` の `results` が `try` の内側で宣言されているため、Whisper がキャンセル例外を投げると**完了済み区間の行まで破棄される**。それらは `SegmentTranscribed` で画面には既に出ているので、`.txt` と画面が食い違う。T112 以前からある問題だが、1 チャンクが複数区間に分かれるようになり影響範囲が広がった。T112 のレビューで発見
