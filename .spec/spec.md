...
### User Story 11 - 文字起こしにGPUを使用するかどうかを切り替える (Priority: P3)

環境によっては GPU を使わずに CPU のみで文字起こしを行いたい（GPU を他用途で専有したい、GPU 利用時の動作が不安定、等）。そのため GPU 利用の ON/OFF をユーザーが選択できるようにしたい。

**Acceptance Scenarios (Given/When/Then)**

- Given: アプリを起動している
- When: 画面を見る
- Then: 「音声ファイルから文字起こし」ボタンの右に「文字起こしにGPUを使用する」チェックボックスが表示され、初期状態は ON になっている

- Given: システムに CUDA / Vulkan 等の GPU ランタイムが利用不可であることが判明している
- When: アプリを起動する
- Then: 「文字起こしにGPUを使用する」チェックボックスは OFF の状態で表示され、操作できないように無効化される

- Given: 「文字起こしにGPUを使用する」チェックボックスが操作可能な状態である
- When: 「文字起こしにGPUを使用する」チェックボックスの ON/OFF を切り替える
- Then: 現在読み込まれている Whisper モデルを破棄し、変更後の設定（GPU 使用有無）でモデルを再読み込みする

- Given: 録音中、または音声ファイルからの文字起こし処理中である
- When: 「文字起こしにGPUを使用する」チェックボックスを操作しようとする
- Then: 「文字起こしにGPUを使用する」チェックボックスは無効化されており、状態を変更できない

**Edge Cases**
- 「文字起こしにGPUを使用する」チェックボックス変更によるモデル再読み込みが失敗した場合 → エラーメッセージを表示し、モデル未読み込み状態として扱う
- モデル再読み込み中に録音・ファイル文字起こしを開始しようとした場合 → 再読み込み完了まで各操作ボタンを無効化する
- GPU 利用可否の判定自体ができない環境 → 「文字起こしにGPUを使用する」チェックボックスは ON（利用可能とみなす）のまま扱い、実際のランタイム選択結果（CPU フォールブック等）は User Story 10 の仕組みに従う
...
- FR-024: 「音声ファイルから文字起こし」ボタンの右に「文字起こしにGPUを使用する」チェックボックスを表示する
- FR-025: 「文字起こしにGPUを使用する」チェックボックスの初期状態は ON（GPU使用）とする
- FR-026: 起動時に GPU（CUDA、Vulkan 等）が利用不可と判定された場合、「文字起こしにGPUを使用する」チェックボックスを OFF にしたうえで無効化する
- FR-027: 「文字起こしにGPUを使用する」チェックボックスの状態変更時、Whisper モデルを破棄し、変更後の設定で再読み込みする
- FR-028: 録音中、または音声ファイルからの文字起こし中は、「文字起こしにGPUを使用する」チェックボックスの操作を禁止する（無効化する）
...
- `UseGpuForTranscription` (bool): 文字起こしに GPU を使用するか（既定値: true）
...
- SC-014: GPU が利用不可の環境では、アプリ起動時点で「文字起こしにGPUを使用する」チェックボックスが OFF・無効化された状態になっている
- SC-015: 「文字起こしにGPUを使用する」チェックボックス変更後のモデル再読み込みが完了するまで、録音・ファイル文字起こしの開始操作が無効化される
- SC-016: 録音中または文字起こし中は、「文字起こしにGPUを使用する」チェックボックスがクリックできない（無効化されている）
